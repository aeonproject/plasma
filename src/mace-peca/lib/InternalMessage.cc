...
const uint8_t mace::ElasticityControl_Message::LEM_INFO_REPORT;
...
const uint8_t mace::AllocateContextObjReq_Message::ALLOCATE_CTX_OBJ_REQ;
const uint8_t mace::AllocateContextObjReq_Message::NEW_CTX_ADDR_REQ;
...
void mace::TransferContext_Message::serialize(std::string& str) const {
    ...
    mace::serialize(str, &ownershipVers);
...
int mace::TransferContext_Message::deserialize(std::istream& __mace_in) throw (mace::SerializationException) { 
    ...
    serializedByteSize +=  mace::deserialize(__mace_in, &ownershipPairs);
    serializedByteSize +=  mace::deserialize(__mace_in, &ownershipVers);
