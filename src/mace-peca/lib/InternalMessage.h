...
namespace mace {
  ...
  public:
	...
	public:
    const static uint8_t NEW_CTX_ADDR_REQ = 1;
    const static uint8_t ALLOCATE_CTX_OBJ_REQ = 2;
  ...
  private:
	...
	public:
		AllocateContextObjReq_Message(): _data_store_( new AllocateContextObjReq_struct() ), 
			serializedByteSize(0),
      mType( _data_store_->mType ),
      destAddr( _data_store_->destAddr ),
      srcAddr( _data_store_->srcAddr ), 
      ...
      vers( _data_store_->vers ) {}
		...
		AllocateContextObjReq_Message( const uint8_t& mType, const mace::MaceAddr& destAddr, const mace::MaceAddr& srcAddr, 
      const mace::string& ctxName, const OrderID& eventId, const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs,
      const mace::map< mace::string, uint64_t>& vers  ): 
      _data_store_(NULL), serializedByteSize(0), mType(mType), destAddr(destAddr), srcAddr(srcAddr), createContextName(ctxName), eventId(eventId), 
      ownershipPairs(ownershipPairs), vers(vers) {}

		AllocateContextObjReq_Message( InternalMessageHelper const*o): _data_store_( new AllocateContextObjReq_struct() ), 
			 serializedByteSize(0), mType(_data_store_->mType), destAddr(_data_store_->destAddr), srcAddr(_data_store_->srcAddr), 
       createContextName(_data_store_->createContextName), eventId(_data_store_->eventId),
      ...
      _data_store_->mType = orig_helper->mType;
      ...
      serializedByteSize += mace::deserialize(__mace_in, &_data_store_->mType);
...
class ContextMigrationRequest_Message: public InternalMessageHelper, virtual public PrintPrintable{
  ...
  public:
    static const uint8_t LEM_INFO_REPORT = 1;
    static const uint8_t LEM_INFO_REPORT_REPLY = 6;
    static const uint8_t CONTEXTS_MIGRATION_QUERY = 2;
    static const uint8_t CONTEXTS_MIGRATION_QUERY_REPLY = 3;
    static const uint8_t INACTIVATE_CONTEXT = 4;
    static const uint8_t ACTIVATE_CONTEXT = 5;
    static const uint8_t CREATE_CONTEXT_OBJ = 6;
  ...
  public:
    ElasticityControl_Message( ): _data_store_( new ElasticityControl_struct() ), 
      ...
      contextNames( _data_store_->contextNames ),
      serverInfo( _data_store_->serverInfo ),
      mactions( _data_store_->mactions ),
      contextRuntimeInfos( _data_store_->contextRuntimeInfos ) { }

    ElasticityControl_Message( const uint8_t& controlType, mace::string const& contextName, 
      mace::MaceAddr const& addr, const uint64_t& value, const mace::vector<mace::string>& contextNames, 
      const mace::ServerRuntimeInfo& serverInfo, const mace::vector<mace::ElasticityBehaviorAction>& mactions,
      const mace::vector<mace::ContextRuntimeInfoForElasticity>& contextRuntimeInfos ): _data_store_( NULL ), 
      serializedByteSize(0) , controlType( controlType ), contextName(contextName), addr(addr), 
      value( value ), contextNames( contextNames ), serverInfo( serverInfo ), mactions( mactions),
      contextRuntimeInfos( contextRuntimeInfos ) { }
   
    // WC: C++03 forbids calling a constructor from another constructor.
    ElasticityControl_Message( InternalMessageHelper const* o): _data_store_( new ElasticityControl_struct() ), 
        serializedByteSize(0) , 
        controlType( _data_store_->controlType ), contextName( _data_store_->contextName ),
        addr( _data_store_->addr ), value( _data_store_->value ), contextNames( _data_store_->contextNames ),
        serverInfo( _data_store_->serverInfo ), mactions( _data_store_->mactions), 
        contextRuntimeInfos( _data_store_->contextRuntimeInfos ) {
      ...
      _data_store_->contextNames = orig_helper->contextNames;
      _data_store_->serverInfo = orig_helper->serverInfo;
      _data_store_->mactions = orig_helper->mactions;
      _data_store_->contextRuntimeInfos = orig_helper->contextRuntimeInfos;
    }
    ...
    void serialize(std::string& str) const {
      ...
      mace::serialize(str, &contextNames);
      mace::serialize(str, &serverInfo );
      mace::serialize(str, &mactions);
      mace::serialize(str, &contextRuntimeInfos);
      bool __false = false;
      mace::serialize(str, &__false );
    ...
    int deserialize(std::istream& __mace_in) throw (mace::SerializationException) {
      ...
      serializedByteSize +=  mace::deserialize(__mace_in, &_data_store_->contextNames);
      serializedByteSize +=  mace::deserialize(__mace_in, &_data_store_->serverInfo );
      serializedByteSize +=  mace::deserialize(__mace_in, &_data_store_->mactions);
      serializedByteSize +=  mace::deserialize(__mace_in, &_data_store_->contextRuntimeInfos);
      bool __unused;
      serializedByteSize += mace::deserialize( __mace_in, &__unused );
    ...
    mace::vector<mace::string> const& contextNames;
    mace::ServerRuntimeInfo const& serverInfo;
    mace::vector< mace::ElasticityBehaviorAction> const& mactions;
    mace::vector<mace::ContextRuntimeInfoForElasticity> const& contextRuntimeInfos;
  };
  ...
  class TransferContext_Message: public InternalMessageHelper, virtual public PrintPrintable{
    ...
    mutable std::string serializedCache;
  public:
    TransferContext_Message() : serializedByteSize(0) , ctxParams(NULL), checkpoint(""), eventId( ), ownershipPairs(), ownershipVers() {}
    TransferContext_Message( const ContextBaseClassParams* my_ctxParams, mace::string const & my_checkpoint, mace::OrderID const & my_eventId,
      const mace::vector< mace::pair<mace::string, mace::string> >& my_ownershipPairs, const mace::map< mace::string, uint64_t >& my_ownershipVers) : serializedByteSize(0) {
      
      ctxParams = const_cast<ContextBaseClassParams*>(my_ctxParams);
      checkpoint = my_checkpoint;
      eventId = my_eventId;
      ownershipPairs = my_ownershipPairs;
      ownershipVers = my_ownershipVers;
    }
    ...
    TransferContext_Message(InternalMessageHelper const* o) : serializedByteSize(0) {
     TransferContext_Message const* orig_helper = static_cast< TransferContext_Message const* >( o );
      ...
      ownershipPairs = orig_helper->ownershipPairs;
      ownershipVers = orig_helper->ownershipVers;
    }
  ...
  const static uint8_t COMMIT_CONTEXT_MIGRATION = 44;
    ...
    InternalMessage( AllocateContextObjReq_type t, const uint8_t& mType, const mace::MaceAddr& destAddr, const mace::MaceAddr& srcAddr, 
      const mace::string& ctxName, const OrderID& eventId, const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs,
      const mace::map< mace::string, uint64_t>& vers ): 
      msgType( ALLOCATE_CONTEXT_OBJ_REQ ), msgId( ), helper( new AllocateContextObjReq_Message( mType, destAddr, srcAddr, ctxName, eventId, ownershipPairs, vers ) ) { }
    ...
    InternalMessage( ElasticityControl_type t, InternalMessageID const& msgId, const uint8_t& controlType, 
      mace::string const& contextName, mace::MaceAddr const& addr, const uint64_t& value, const mace::vector<mace::string>& contextNames,
      mace::ServerRuntimeInfo const& serverInfo, mace::vector<mace::ElasticityBehaviorAction> const& mactions,
      mace::vector<mace::ContextRuntimeInfoForElasticity> const& contextRuntimeInfos ): 
      msgType( ELASTICITY_CONTROL), msgId( msgId ), 
      helper( new ElasticityControl_Message(controlType, contextName, addr, value, contextNames, serverInfo, mactions, 
                                            contextRuntimeInfos ) 
      ) { }
    ...
    InternalMessage( TransferContext_type t, const mace::ContextBaseClassParams* my_ctxParams, mace::string const & my_checkpoint, mace::OrderID const & my_eventId,
      const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs, const mace::map< mace::string, uint64_t >& ownershipVers ): msgType( TRANSFER_CONTEXT), 
      msgId( ), helper(new TransferContext_Message( my_ctxParams, my_checkpoint, my_eventId, ownershipPairs, ownershipVers ) ) {}
