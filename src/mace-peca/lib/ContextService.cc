...
using mace::ReadLine;
const uint8_t mace::ElasticityControl_Message::LEM_INFO_REPORT_REPLY;
...
void ContextService::handleInternalMessages( mace::InternalMessage const& message, MaceAddr const& src, uint64_t msg_size ){
  ...
  switch( message.getMessageType() ){
    ...
    case mace::InternalMessage::ALLOCATE_CONTEXT_OBJECT: {
      ...
      if( m->mType == mace::AllocateContextObjReq_Message::NEW_CTX_ADDR_REQ ){
        handle__event_newContextAddrRequest( m->createContextName, m->eventId, m->srcAddr, m->ownershipPairs, m->vers );
      } else if( m->mType == mace::AllocateContextObjReq_Message::ALLOCATE_CTX_OBJ_REQ ) {
        if( m->srcAddr == SockUtil::NULL_MACEADDR) {
          maceerr << "context("<< m->createContextName <<")'s srcAddr is "<< m->srcAddr <<", destAddr: " << m->destAddr << Log::endl;
          ASSERT(false);
        }

        handle__event_allocateContextObjectRequest( m->createContextName, m->eventId, m->srcAddr, m->destAddr, m->ownershipPairs, 
          m->vers );
    ...
    case mace::InternalMessage::TRANSFER_CONTEXT:{
      ...
      handle__event_TransferContext( src, m->ctxParams, m->checkpoint, m->eventId, m->ownershipPairs, m->ownershipVers );
      break;
    }
    ...
    case mace::InternalMessage::ELASTICITY_CONTROL: {
      ...
      handleElasticityControlMessage( m, src );
      break;
    }
...
void ContextService::handleElasticityControlMessage( mace::ElasticityControl_Message* m, const MaceAddr& src ) {
  if( m->controlType == mace::ElasticityControl_Message::LEM_INFO_REPORT ) {
    elasticityMonitor->enqueueLEMReport( m->serverInfo, m->contextRuntimeInfos, src );
  } else if( m->controlType == mace::ElasticityControl_Message::LEM_INFO_REPORT_REPLY ) { 
    elasticityMonitor->processLEMReportReply( m->mactions );
  } else if(m->controlType == mace::ElasticityControl_Message::CONTEXTS_MIGRATION_QUERY){
    elasticityMonitor->enqueueContextMigrationQuery( src, m->mactions, m->serverInfo );
...
void ContextService::handle__event_allocateContextObjectRequest( mace::string const& ctxName, mace::OrderID const& eventId, 
  ...
  HeadEventDispatch::HeadEventTP::executeContextCreateEvent(this, ctxName, eventId, src, new_addr, ownershipPairs, vers);	
}

void ContextService::handle__event_newContextAddrRequest(mace::string const& ctxName, mace::OrderID const& eventId, 
    const mace::MaceAddr& src, const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs, 
    const mace::map< mace::string, uint64_t>& vers) {
  ADD_SELECTORS("ContextService::handle__event_newContextAddrRequest");

  mace::MaceAddr new_ctx_addr = elasticityMonitor->getNewContextAddr( ctxName );
  this->send__event_allocateContextObjectRequest( new_ctx_addr, src, ctxName, eventId, ownershipPairs, vers );
}
...
void ContextService::handle__event_ContextMigrationRequest( MaceAddr const& src, MaceAddr const& dest, mace::Event const& event, 
  ...
  for(; mctxIter != migrateContextIds.end(); mctxIter++) {
    ...
    ctx_names.insert( thisContext->contextName );

    eraseContextData( thisContext );


    mace::vector< mace::pair<mace::string, mace::string> > ownershipPairs = contextStructure.getOwnershipOfContexts( ctx_names );
    ...
    send__event_TransferContext( dest, ctxParams, contextData, event.getEventID(), ownershipPairs, ownershipVers );
...
void ContextService::notifyNewContext(mace::Event & he,  const uint8_t serviceID ) {
    ...
    if( he.eventType == mace::Event::STARTEVENT ){
      ...
      mace::MaceAddr dest_addr = SockUtil::NULL_MACEADDR;
      std::pair< mace::MaceAddr, uint32_t > newMappingReturn = contextMapping.newMapping( globalContextID, dest_addr );
...
void ContextService::trytoCreateNewContextObject( mace::string const& ctx_name, mace::OrderID const& eventId, 
  ...
  const mace::MaceAddr& srcAddr = Util::getMaceAddr();
  
  if( elasticityMonitor->checkContextCreateRule(ctx_name) ) {
    mace::MaceAddr dest_addr = elasticityMonitor->getNewContextAddrForActorRules( ctx_name );

    if( dest_addr != SockUtil::NULL_MACEADDR ) {
      this->send__event_allocateContextObjectRequest( dest_addr, srcAddr, ctx_name, eventId, ownershipPairs, vers );
    } else {
      const mace::MaceAddr& gem_addr = elasticityMonitor->getGEMAddr();
      if( gem_addr == SockUtil::NULL_MACEADDR ) {
        macedbg(1) << "The GEM address is NULL!" << Log::endl;
        this->send__event_allocateContextObjectRequest( SockUtil::NULL_MACEADDR, srcAddr, ctx_name, eventId, ownershipPairs, vers );
      } else {
        macedbg(1) << "Send create request("<< ctx_name <<") to GEM: " << gem_addr << Log::endl;
        this->send__event_newContextAddrRequest( gem_addr, srcAddr, ctx_name, eventId, ownershipPairs, vers );
      }
    }
  } else {
    this->send__event_allocateContextObjectRequest( SockUtil::NULL_MACEADDR, srcAddr, ctx_name, eventId, ownershipPairs, vers );
...
void ContextService::snapshotContext( const mace::string& fileName ) {
  mace::ContextBaseClass* thisContext = ThreadStructure::myContext();
  ASSERT( thisContext != NULL );
  ...
  writer << contextSnapshot << std::endl;
  
  mace::vector<mace::string> children = contextStructure.getAllChildContexts( thisContext->contextName );
  mace::string ownershipSnapshot;
  serialize( ownershipSnapshot, &children );

  // thisContext->snapshot( contextSnapshot );
  writer << ownershipSnapshot << std::endl;
  writer.close();
}

void ContextService::restoreContext( const mace::string& fileName ) {
  mace::ContextBaseClass* thisContext = ThreadStructure::myContext();
  ASSERT( thisContext != NULL );

  mace::string contextSnapshot, ownershipSnapshot;
  std::ifstream reader(fileName.c_str());

  std::getline( reader, contextSnapshot );
  std::getline( reader, ownershipSnapshot );

  mace::deserialize( contextSnapshot, thisContext );

  mace::vector<mace::string> children;
  mace::deserialize( ownershipSnapshot, &children );

  const mace::string& contextName = thisContext->contextName;
  for( uint32_t i=0; i<children.size(); i++ ) {
    createNewOwnership( contextName, children[i] );
  }
}
...
bool ContextService::addNewCreateContext( mace::string const& ctx_name, const mace::MaceAddr& src) {
  ...
  if( src == SockUtil::NULL_MACEADDR ) {
    maceerr << "Add new create context: " << ctx_name << " from " << src << Log::endl;
    ASSERT(false);
  }
...
void ContextService::notifyContextMappingUpdate(mace::string const& ctxName) {
  ...
  if( iter == processContextCreate.end() ) {
    ...
    for( mace::vector<mace::MaceAddr>::const_iterator addr_iter = addrs.begin(); addr_iter != addrs.end(); addr_iter ++) {
      if( *addr_iter == SockUtil::NULL_MACEADDR) {
        ASSERT(false);
      }
...
void ContextService::notifyContextMappingUpdate(mace::string const& ctxName, const mace::MaceAddr& src) {
  ADD_SELECTORS("ContextService::notifyContextMappingUpdate#2");
...
void ContextService::send__event_commit( MaceAddr const& destNode, mace::Event const& event, mace::string const& ctxName ){
  ...
  void ContextService::send__event_allocateContextObjectRequest( const mace::MaceAddr& new_ctx_addr, const mace::MaceAddr& src_addr, 
      mace::string const& ctx_name, mace::OrderID const& eventId, 
      const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs, const mace::map< mace::string, uint64_t>& vers ) {

    ADD_SELECTORS("ContextService::send__event_allocateContextObjectRequest");

    mace::InternalMessage msg( mace::AllocateContextObjReq, mace::AllocateContextObjReq_Message::ALLOCATE_CTX_OBJ_REQ, new_ctx_addr, 
      src_addr, ctx_name, eventId, ownershipPairs, vers );

    const mace::ContextMapping& snapshot = this->getLatestContextMapping();
    const mace::MaceAddr& headnode = mace::ContextMapping::getHead( snapshot );
    forwardInternalMessage( headnode, msg);
  }

  void ContextService::send__event_newContextAddrRequest( const mace::MaceAddr& gem_addr, const mace::MaceAddr& src_addr, 
      mace::string const& ctx_name, const mace::OrderID& eventId, 
      const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs, 
      const mace::map< mace::string, uint64_t>& vers ) {

    mace::InternalMessage msg( mace::AllocateContextObjReq, mace::AllocateContextObjReq_Message::NEW_CTX_ADDR_REQ, gem_addr, src_addr, 
      ctx_name, eventId, ownershipPairs, vers );
    forwardInternalMessage( gem_addr, msg );
  }
...
// Elasticity methods
void ContextService::reportLEMInfos( const mace::MaceAddr& dest, const mace::ServerRuntimeInfo& server_info, 
  ...
  mace::InternalMessage msg( mace::ElasticityControl, msgId, mace::ElasticityControl_Message::LEM_INFO_REPORT, 
    ctx_name, addr, value, ctx_names, server_info, mactions, ctx_rt_infos);
  forwardInternalMessage( dest, msg);

}

void ContextService::replyLEMReport( const mace::MaceAddr& dest, const mace::vector<mace::ElasticityBehaviorAction>& mactions) const {
  mace::string ctx_name;
  mace::MaceAddr addr;
  uint64_t value;
  mace::vector<mace::string> ctx_names;
  mace::ServerRuntimeInfo server_info;
  mace::vector<mace::ContextRuntimeInfoForElasticity> ctx_rt_infos;

  mace::InternalMessageID msgId( Util::getMaceAddr(), "", 0 );

  mace::InternalMessage msg( mace::ElasticityControl, msgId, mace::ElasticityControl_Message::LEM_INFO_REPORT_REPLY, 
    ctx_name, addr, value, ctx_names, server_info, mactions, ctx_rt_infos);
  forwardInternalMessage( dest, msg);
...
void ContextService::send__elasticity_contextMigrationQuery( const mace::MaceAddr& dest, 
    const mace::vector< mace::ElasticityBehaviorAction>& query_mactions, const mace::ServerRuntimeInfo& server_info ) const {
  ...
  mace::InternalMessage msg( mace::ElasticityControl, msgId, mace::ElasticityControl_Message::CONTEXTS_MIGRATION_QUERY, 
    ctx_name, addr, value, ctx_names, server_info, query_mactions, ctx_rt_infos);
  forwardInternalMessage( dest, msg);
}
...
void ContextService::send__elasticity_contextMigrationQueryReply( const mace::MaceAddr& dest, 
  ...
  mace::InternalMessage msg( mace::ElasticityControl, msgId, mace::ElasticityControl_Message::CONTEXTS_MIGRATION_QUERY_REPLY, 
    ctx_name, addr, value, accept_m_ctxs, server_info, mactions, ctx_rt_infos);
  forwardInternalMessage( dest, msg);
}
...
void ContextService::inactivateContext( const mace::string& context_name) {
  ...
  mace::InternalMessage msg( mace::ElasticityControl, msgId, mace::ElasticityControl_Message::INACTIVATE_CONTEXT, 
    context_name, addr, value, ctx_names, server_info, mactions, ctx_rt_infos);
...
void ContextService::send__event_TransferContext( MaceAddr const& dest, const mace::ContextBaseClassParams* ctxParams, 
    mace::string const& checkpoint, mace::OrderID const eventId, const mace::vector< mace::pair<mace::string, mace::string> >& ownershipPairs, 
    const mace::map< mace::string, uint64_t >& ownershipVers ){

  mace::InternalMessage msg( mace::TransferContext, ctxParams, checkpoint, eventId, ownershipPairs, ownershipVers );
  forwardInternalMessage( dest, msg );
}
...
void ContextService::handleContextMigrationRequest( MaceAddr const& dest, mace::string const& contextName, 
  ...
  eraseContextData( thisContext );
  macedbg(1) << "Erased context " << contextName << Log::endl;

  mace::set<mace::string> ctx_names;
  ctx_names.insert( contextName );

  mace::vector<mace::string> p_contexts = contextStructure.getAllParentContexts( contextName );
  for( uint32_t i=0; i<p_contexts.size(); i++ ) {
    ctx_names.insert( p_contexts[i] );
  }
  ...
  // macedbg(1) << "ownershipPairs: " << ownershipPairs << ", ownershipVers: " << ownershipVers << Log::endl;
  macedbg(1) << "Send context("<< contextName <<") to new node("<< dest <<")!" << Log::endl;
  
  this->send__event_TransferContext( dest, ctxParams, contextData, mEvent.eventId, ownershipPairs, ownershipVers );
}
...
void ContextService::handle__event_TransferContext( MaceAddr const& src, const mace::ContextBaseClassParams* ctxParams, 
  ...
  // macedbg(1) << "Received a context from " << src << ", with ownershipPairs("<< ownershipPairs <<") and ownershipVers("<< ownershipVers <<")!" << Log::endl;
  ...
  contextStructure.getLock(ContextStructure::WRITER);
  contextStructure.updateOwnerships( ownershipPairs, ownershipVers );
  contextStructure.releaseLock(ContextStructure::WRITER);
